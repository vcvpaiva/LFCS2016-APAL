...
this map is $F : U \times Y \to X$.  The difference in the definition
of morphims may seem small, but while the modification prevents the
definition of a second monoidal structure modeling the connective par
in the category $\dcSets{2}$, it is this category that has a free
comonoid structure, the only non merely syntactic example in the
...
applications in concurrency.  The category $\dial$ shares many of the
properties of Chu spaces on sets and on $\mathsf{2}$
\cite{dePaiva:2007}.  Pratt and Gupta \cite{Gupta:1994} showed that
Chu spaces can be seen as a model of concurrency, but in classical
linear logic, while dialectica spaces should provide a model of
concurrency in full intuitionistic linear logic.
