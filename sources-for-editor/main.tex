...
\begin{abstract}
  ...
  cut-elimination by Bierman, but Bellin proposed a small change to
  the definition of FILL allowing to regain cut-elimination, using
  ...
  tensor model of Melli\`es' and Tabareau's tensorial logic.  We give
  a double-negation translation of linear logic into FILL that
...
extracted a general library for proving properties of dialectica
...
\section{The Agda Formalization: A Library for Studying Dialectica Categories}
...
categories in sets that we believe may be of interest to the wider
logical community.  This section introduces this library, but before
...
A lineale is essentially a symmetric monoidal category in the category
...
\end{definition}
Now a lineale can be seen as essentially a closed symmetric monoidal
...
prosets, but our formalization given here shows that anti-symmetry can
...
this map is $F : U \times Y \to X$.  This small modification is enough
to prevent the ability to define a second monoidal structure to model
the linear connective par.
...
category $\dial$ has a lot in common with Chu construct on sets and on
...
logic, while dialectica spaces should provide a model of currency in
intuitionistic linear logic.
...
\bibitem{dePaiva:2006} de~Paiva V.: Dialectica and Chu Constructions:
...
\bibitem{Hyland:1991} Hyland M., de~Paiva V.: Lineales. In "O que nos
